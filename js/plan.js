...
function TimeTrip(LocationName, routes, Grade, period, date, type, distance, time, startingTime, size, discription, food) {
...
tripFood.addEventListener("change", function () { // modify the select options depending on the selected filters
    ...
    if (container !== null) {
        ...
        for (var i = 0; i < newlocation.length; i++) { // iterate over all locations
            // check if the filters match the location, and display it
            if (tripFood.value == newlocation[i].food) {
                var opt = document.createElement('option');
                opt.appendChild(document.createTextNode(newlocation[i].food));
                opt.value = newlocation[i].food;
                container.appendChild(opt);
            }
